...
| A28 | **Library richness / plentiful inventory** — quantitative variety across the bank: total sample count, duration distribution (short/medium/long bucket coverage), mood/affect distribution (≥3 distinct affects tagged), logical-event coverage (audio fires on multiple distinct game events, not just one). The Hanna-Barbera Sound Effects Library shipped 600+ named effects across decades; modern indie libraries target 30-100 with documented duration and mood distribution. | Aaron Marks, *The Complete Guide to Game Audio* 2nd ed. (Focal 2008) ch. 11 on library construction; Sound Ideas Hanna-Barbera Sound Effects Library reissue (Sound Ideas 1993) catalog as an industry reference standard. |
...
| **Variety & Game Feel** | Variant pools per logical event (≥3 variants), no-immediate-repeat / shuffle-bag selection, pitch/gain jitter, input-to-audio latency under 50ms, audio coverage of player-input events ≥70%. **Library richness sub-test (A28):** ≥20 unique samples for "shipping" tier OR ≥3 variants × ≥6 logical events; duration distribution covers ≥2 of {short ≤0.5s, medium 0.5-1.5s, long >1.5s}; mood/affect distribution covers ≥3 named affects (e.g. comedic / triumphant / embarrassed / sneaky / surprised) tagged in the manifest; logical-event count ≥3 distinct named events that fire audio. | A5, A6, A28 |
...
- **Variety & Game Feel: 3.** Procedural synthesis varies durations and oscillator counts based on slider input. Pitch bends are randomized via `Math.random()` at [src/audio/procedural.ts:41](src/audio/procedural.ts:41). However, no variant *pool* per logical event — same parameters always produce the *same waveform skeleton*; A5's no-immediate-repeat / shuffle-bag pattern is absent. Input-to-audio latency under 100ms (single function call inside click handler). Coverage of input events: only Launch produces audio; the Random button does (it just clicks Launch); slider-drag produces no audio (could be a small juice opportunity). Coverage ~50%. **Library richness (A28): zero unique samples (procedural-only); duration distribution n/a (single function output); mood/affect distribution n/a (no manifest); logical-event count = 1 (only Launch fires audio).** This drops the axis 2 points from v3.0's 5 to v3.1's 3 — the prior score didn't fully account for the one-event-only design.
...
Average: (7+3+3+4+4+2)/6 = 23/6 = **3.83**.
...
5. **Replace the parameter-space synth with a named-preset library** (Stalling A21, Collins A26, Marks A28). Define ≥20 named effects across ≥6 logical events (Launch + slider-drag + scoring-feedback + achievement-unlock + combo-streak + onboarding-step). Tag each with a mood/affect label from {comedic, triumphant, embarrassed, sneaky, surprised, exhausted}; require ≥3 affects represented. Cover all three duration buckets ({short ≤0.5s, medium 0.5-1.5s, long >1.5s}) so the library has rhythmic range. Each preset is a distinct *spectral shape* (different waveform, filter chain, envelope), not a different point in the same parameter space. Pass criterion: blind-listening test on 20 random triggers gets ≥8 distinct labels from a stranger AND duration histogram covers all 3 buckets.
