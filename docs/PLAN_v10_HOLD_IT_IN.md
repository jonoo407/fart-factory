...
| D2 | **Risk model** *(amended in P0)* | Per-bite seeded roll at stuff time: `bustChance = RISK_BY_PRESSURE[pressureAfter] × size² × venueRiskMod`. **size² is load-bearing** (the Pyramid theorem, tested); **the per-pressure slope WITHIN zones is equally load-bearing** — the first gate run proved a flat per-zone rate makes crumb-topping to 18 free and "always summit" a solved threshold (Gate 2 read 0% edge). Final tuned table lives in `src/v10/tuning.ts`; the gates are the tuner of record. |
...
| D5 | **Crowd taste from existing cravings** *(amended in P0)* | Top-2 tag-axes with craving ≥ 3 → **LOVES**; `no-X`/`max-X` restrictions → at most one **HATE**. Zone demands come in TWO kinds: **MIN** ("EPIC or the Frat Bros boo") and **CAP** ("keep it UNDER: SOLID" — the Library, Granny). Meeting a cap pays ×CAP_MET_BONUS(2) — restraint is a skill; blowing past one is a scandal (÷4). Caps are required for Gate 2: without them max-pushing is universally right. Assignment by `difficultyTier` + crowd fiction; all 20 audiences survive. |
| D6 | **Applause** *(amended in P0)* | `bankedPressure × zoneMult × (1 + 0.5·lovedTagHits) × demandFactor × venueMod`, plus the **NUCLEAR bonus**: banking ≥17 pays ×1.5. Pressure scales WITHIN zones (flat-per-zone pay made "stop at the zone line" optimal), and NUCLEAR makes the summit squeeze EV-positive (without it the expert's optimal bust rate was 9.8% — under the excitement band; raw risk increases only make optimal play *more* cautious). Unmet MIN ×0.5; HATE tag / exceeded CAP → SCANDAL ÷4 + card. Bust → Disaster Card + pity (never zero). Encore falloff ×0.75ⁿ floor ×0.25. Gold = applause 1:1; FAME += applause, never decreases. |
