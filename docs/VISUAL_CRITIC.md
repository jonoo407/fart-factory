...
| V27 | **Reactive variety per game state** — the UI surface visibly reacts differently across game-state tiers (score brackets, streak levels, achievement unlocks, hidden states). Same input in different contexts should look different; a "near-miss" should feel different from a "fail," a "low score" different from a "high score," a "first try" different from a "tenth try." Hidden states (Easter eggs, threshold-triggered surprises) reward exploration. | Steve Swink, *Game Feel* (Morgan Kaufmann/CRC 2008) ch. 4 on responsive systems; Jesse Schell, *The Art of Game Design* Lens of Reactivity (Lens #54 in 3rd ed.) and Lens of Surprise (Lens #4); Don Norman, *Design of Everyday Things* (rev. 2013) on system-state communication. |
...
| **Hierarchy & Affordance** | One visually-dominant primary CTA per screen; every clickable has a signifier (button shape, cursor:pointer, icon); consistent grouping per Gestalt proximity/similarity. **Reactive variety sub-test (V27):** UI visibly differentiates across ≥3 score-bracket tiers; ≥3 streak-tier responses (1-streak / 3-streak / 5-streak distinct); ≥1 hidden state (Easter egg or threshold surprise) rewards exploration; near-miss states (e.g. "1 point from S+") render visibly differently from full-fail or full-pass. | V10, V11, V27 |
...
- **Hierarchy & Affordance: 8.** Launch is the visually-dominant CTA (1.4em font, gradient background, larger boxshadow). Random is secondary. Sliders have signifiers (gradient track, white circular thumb, focus ring). Onboarding tutorial uses one Next per step. ✓ on the BBC-GEL "one primary action" rule. **Reactive variety (V27): genuinely good for the current scope.** Score brackets ([src/content/commentary.ts:51-58](src/content/commentary.ts#L51)) drive 6 distinct emoji reaction sets. S+ triggers sparkle burst at [src/visuals/particles.ts](src/visuals/particles.ts). total > 40 triggers `body` shake at [src/main.ts:125](src/main.ts:125). Combo banner appears at streak ≥3 ([src/main.ts:58](src/main.ts:58)). Achievement toasts unlock at thresholds. Stink fill width varies with stinkiness. ≥5 distinct visible response tiers — passes V27's ≥3 score-bracket and ≥3 streak-tier sub-tests. Missing: explicit near-miss state (e.g. "1 point from S+" → near-miss visual), explicit hidden Easter eggs at unusual slider configurations. Axis bumps from v3.0's 7 to v3.1's 8 — the prior score didn't credit the existing reactive variety.
...
Average: (7+9+7+9+7+6+8+6+3) / 9 = 62/9 = **6.89**.
